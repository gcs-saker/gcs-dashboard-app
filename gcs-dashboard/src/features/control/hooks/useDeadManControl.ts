...
import type { ControlIntentSender } from "@control/api/controlIntentSender";
import type { MotionAxes } from "@control/contracts/controlIntent";
...
export function useDeadManControl(enabled: boolean, send: ControlIntentSender) {
...
function axesFor(keys: Set<string>): MotionAxes {
  ...
  for (const key of keys) {
    for (const [axis, value] of Object.entries(KEY_AXES[key])) {
      axes[axis as keyof MotionAxes] += value ?? 0;
    }
  }
  return axes;
}

function isStopped(axes: MotionAxes): boolean {
  return Object.values(axes).every((value) => value === 0);
}
