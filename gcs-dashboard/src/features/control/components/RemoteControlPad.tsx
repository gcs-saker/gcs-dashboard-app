...
export function RemoteControlPad({ enabled, onIntent }: RemoteControlPadProps) {
  ...
  const button = (key: string, label: string) => (
    <button
      ...
      disabled={!enabled}
      aria-label={label}
      onPointerDown={() => control.press(key)}
      onPointerUp={() => control.release(key)}
      onPointerCancel={control.stop}
      onPointerLeave={() => control.release(key)}
    >
      {key.toUpperCase()}
    </button>
  );
  return (
    <section aria-label="원격 조종" data-control-enabled={enabled}>
      ...
      <button type="button" disabled={!enabled} onClick={control.stop}>
        정지
      </button>
    </section>
  );
