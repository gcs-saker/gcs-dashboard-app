...
func ParseStreamPath(path string) (ParsedStreamPath, error) {
	...
	return parseStreamPathSegments(parts)
}

func parseStreamPathSegments(parts []string) (ParsedStreamPath, error) {
	switch parts[0] {
	case "raw":
		return parseRawStreamPath(parts)
	case "ai":
...
func parseAIStreamPath(parts []string) (ParsedStreamPath, error) {
	if len(parts) != 4 {
...
func parseArchiveStreamPath(parts []string) (ParsedStreamPath, error) {
	if len(parts) != 4 {
