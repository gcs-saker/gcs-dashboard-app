...
def test_security_testing_profile_tracks_implemented_and_unrun_work_separately() -> None:
    ...
    assert profile["dynamic"]["dast"]["status"] == "IMPLEMENTED_DISPOSABLE_PASS"
    assert profile["dynamic"]["dast"]["lastDisposableResult"] == {
        "scenarios": 11,
        "passed": 11,
        "leakageDetected": 0,
    }
...
def test_fuzz_targets_exist_in_owned_boundary_packages() -> None:
    targets = {
        ...
        "FuzzGatewayMetadata": REPO_ROOT / "services/media-control/internal/grpcgateway/auth_fuzz_test.go",
        "FuzzSessionFromTopic": REPO_ROOT / "services/media-control/internal/mqttgateway/message_fuzz_test.go",
    }
