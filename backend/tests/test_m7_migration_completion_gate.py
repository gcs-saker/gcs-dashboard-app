...
def test_m7_completion_gate_isolates_legacy_and_future_python_paths() -> None:
    ...
    for legacy_term in [
        ...
        "`/api/control/*` | Python backend | 실제 장비 제어 정책이 확정되지 않은 future command 기능",
        "`/api/v1/ai/mock/detections` | Python backend | 실제 AI overlay server 연동 전 mock contract",
        "`/metrics` | Python backend | 신규 서비스별 metrics 설계 전까지 legacy observation",
        "`/ws/*` | Python backend | WebSocket contract가 확정되지 않은 future/legacy path",
...
def test_nginx_contract_keeps_legacy_backend_fallbacks_outside_active_gate() -> None:
    ...
    assert "Future/legacy command fallback. Real command policy is not part of the M7 migration gate." in config
