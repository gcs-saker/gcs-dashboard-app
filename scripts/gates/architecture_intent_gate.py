...
INTENT_MATRIX = (
    REPO_ROOT / "docs" / "architecture" / "GCS-Saker_design_intent_matrix.yml"
)
RUNTIME_STATUS = (
    REPO_ROOT / "docs" / "architecture" / "GCS-Saker_runtime_stack_status.yml"
)
...
def require_schema(document: dict[str, Any], expected: str, path: Path) -> None:
    ...
    if actual != expected:
        raise ArchitectureIntentError(
            f"{path.relative_to(REPO_ROOT)} schemaVersion must be {expected}, got {actual}"
        )
...
def assert_evidence_paths(intent_id: str, evidence_paths: list[str]) -> int:
    for evidence in evidence_paths:
        ...
        if not path.exists():
            raise ArchitectureIntentError(
                f"{intent_id}: evidence path does not exist: {evidence}"
            )
...
def assert_required_texts(intent_id: str, required_texts: list[dict[str, str]]) -> int:
    for item in required_texts:
        ...
        if text not in haystack:
            raise ArchitectureIntentError(
                f"{intent_id}: required text not found in {item['path']}: {text}"
            )
    return len(required_texts)


def assert_forbidden_texts(
    intent_id: str, forbidden_texts: list[dict[str, str]]
) -> int:
    for item in forbidden_texts:
        ...
        if text.lower() in haystack.lower():
            raise ArchitectureIntentError(
                f"{intent_id}: forbidden text found in {item['path']}: {text}"
            )
    return len(forbidden_texts)


def assert_runtime_statuses(
    intent_id: str, expected_statuses: dict[str, str], runtime_status: dict[str, Any]
) -> int:
    ...
    for stack_name, expected_status in expected_statuses.items():
        ...
        if not isinstance(entry, dict):
            raise ArchitectureIntentError(
                f"{intent_id}: runtime stack is missing: {stack_name}"
            )
...
def assert_compose_active_services(
    intent_id: str, service_names: list[str], compose: dict[str, Any]
) -> int:
    ...
    for service_name in service_names:
        ...
        if not isinstance(service, dict):
            raise ArchitectureIntentError(
                f"{intent_id}: compose service is missing: {service_name}"
            )
        if "profiles" in service:
            raise ArchitectureIntentError(
                f"{intent_id}: active service must not require a profile: {service_name}"
            )
    return len(service_names)


def assert_compose_profile_services(
    intent_id: str, expected_profiles: dict[str, str], compose: dict[str, Any]
) -> int:
    services = compose.get("services", {})
    if not isinstance(services, dict):
        raise ArchitectureIntentError("compose file must contain services")
    ...
    for service_name, expected_profile in expected_profiles.items():
        ...
        if not isinstance(service, dict):
            raise ArchitectureIntentError(
                f"{intent_id}: compose profile service is missing: {service_name}"
            )
...
def assert_nginx_routes(
    intent_id: str, routes: list[dict[str, str]], nginx: str
) -> int:
    for route in routes:
        ...
        if not any(marker in nginx for marker in route_markers):
            raise ArchitectureIntentError(
                f"{intent_id}: nginx route is missing: {route_path}"
            )

        proxy_pass = route["proxyPass"]
        if f"proxy_pass {proxy_pass}" not in nginx:
            raise ArchitectureIntentError(
                f"{intent_id}: nginx proxy target is missing: {proxy_pass}"
            )
    return len(routes)
...
def run_gate() -> GateResult:
    ...
    if not isinstance(intents, list) or not intents:
        raise ArchitectureIntentError(
            "design intent matrix must contain at least one intent"
        )
    ...
    for intent in intents:
        ...
        checked_assertions += assert_evidence_paths(
            intent_id, assertions.get("evidencePaths", [])
        )
        checked_assertions += assert_required_texts(
            intent_id, assertions.get("requiredTexts", [])
        )
        checked_assertions += assert_forbidden_texts(
            intent_id, assertions.get("forbiddenTexts", [])
        )
        ...
        checked_assertions += assert_nginx_routes(
            intent_id, assertions.get("nginxRoutes", []), nginx
        )

    return GateResult(
        checked_intents=len(intents), checked_assertions=checked_assertions
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate GCS-Saker design intent against repository contracts."
    )
    parser.add_argument(
        "--json", action="store_true", help="Print machine-readable gate result."
    )
    return parser
