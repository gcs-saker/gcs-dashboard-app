...
usage() {
  cat <<'EOF'
Usage:
  scripts/gates/m7_regression_gate.sh [--check|--full]
...
check_contracts() {
  ...
  test -f "${REPO_ROOT}/scripts/gates/architecture_intent_gate.py"
  test -f "${REPO_ROOT}/scripts/smoke/m7_single_node_runtime_smoke.sh"
  test -f "${REPO_ROOT}/scripts/smoke/m7_publish_play_smoke.sh"
  test -f "${REPO_ROOT}/scripts/smoke/m7_dashboard_first_frame_smoke.sh"
  test -f "${REPO_ROOT}/scripts/benchmarks/m7_performance_benchmark_matrix.py"
  test -f "${REPO_ROOT}/scripts/smoke/grpc_runtime_smoke.py"
  test -f "${REPO_ROOT}/scripts/smoke/dragonfly_profile_smoke.py"
  test -f "${REPO_ROOT}/scripts/smoke/postgis_runtime_smoke.py"
  test -f "${REPO_ROOT}/scripts/gates/closed_network_static_check.py"
  ...
  "${PYTHON_BIN}" "${REPO_ROOT}/scripts/gates/architecture_intent_gate.py" --json >/dev/null
  "${PYTHON_BIN}" "${REPO_ROOT}/scripts/smoke/grpc_runtime_smoke.py" --check >/dev/null
  "${PYTHON_BIN}" "${REPO_ROOT}/scripts/smoke/dragonfly_profile_smoke.py" --check >/dev/null
  "${PYTHON_BIN}" "${REPO_ROOT}/scripts/smoke/postgis_runtime_smoke.py" --check >/dev/null
...
run_check() {
  ...
  run_step "closed-network static check" "$PYTHON_BIN" "${REPO_ROOT}/scripts/gates/closed_network_static_check.py"
...
run_smoke_full() {
  run_step "single-node runtime smoke check" "${REPO_ROOT}/scripts/smoke/m7_single_node_runtime_smoke.sh" --check
  run_step "publish/play smoke check" "${REPO_ROOT}/scripts/smoke/m7_publish_play_smoke.sh" --check
  run_step "dashboard first-frame smoke check" "${REPO_ROOT}/scripts/smoke/m7_dashboard_first_frame_smoke.sh" --check
}
