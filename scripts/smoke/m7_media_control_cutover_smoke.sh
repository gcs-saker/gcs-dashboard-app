...
usage() {
  cat <<'EOF'
Usage:
  scripts/smoke/m7_media_control_cutover_smoke.sh [--check|--run]
...
run_check() {
  bash -n "$0"
  grep -q "func ParseStreamID" "${REPO_ROOT}/services/media-control/internal/domain/stream_path.go"
  ...
  grep -q "AUTH_POLICY_BASE_URL" "${REPO_ROOT}/services/media-control/cmd/media-control/runtime_env_contract.go"
...
run_live() {
  ...
  if [[ "$START_STACK" == "1" ]]; then
    STOP_STACK=0 "${REPO_ROOT}/scripts/smoke/m7_single_node_runtime_smoke.sh" --run
  fi
