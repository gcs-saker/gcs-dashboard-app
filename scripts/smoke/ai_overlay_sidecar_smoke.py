...
async def run_mock_overlay_smoke() -> dict[str, Any]:
    ...
    decoded_events = tuple(
        AiOverlayEventPayload.from_protobuf_wire(event.to_protobuf_wire())
        for event in events
    )
    ...
    return {
        ...
        "dashboardDtoFields": sorted(
            restored_response.model_dump(by_alias=True).keys()
        ),
        "firstEvent": {
            ...
            "observedUnixMillis": decoded_events[0].observed_unix_millis
            if decoded_events
            else None,
            "receivedUnixMillis": decoded_events[0].received_unix_millis
            if decoded_events
            else None,
        },
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate mock AI overlay sidecar metadata path."
    )
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument(
        "--check", action="store_true", help="Print the stable smoke contract."
    )
    mode.add_argument(
        "--run", action="store_true", help="Run the in-process mock sidecar smoke."
    )
...
def main() -> int:
    args = parse_args()
    payload = (
        build_check_report() if args.check else asyncio.run(run_mock_overlay_smoke())
    )
