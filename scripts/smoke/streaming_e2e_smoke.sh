...
usage() {
  cat <<'EOF'
Usage:
  scripts/smoke/streaming_e2e_smoke.sh [--check|--run]
...
run_check() {
  ...
  bash -n "${REPO_ROOT}/scripts/smoke/streaming_e2e_smoke.sh"
  python3 "${REPO_ROOT}/scripts/smoke/webrtc_ice_smoke.py" --check

  local dry_run
  dry_run="$("${REPO_ROOT}/scripts/smoke/publish_sample_stream.sh" --dry-run)"
...
run_smoke() {
  ...
  "${REPO_ROOT}/scripts/smoke/publish_sample_stream.sh" --duration "$SMOKE_DURATION_SECONDS" &
  PUBLISH_PID="$!"
  ...
  if [[ "$WEBRTC_ICE_SMOKE" == "1" ]]; then
    ...
    python3 "${REPO_ROOT}/scripts/smoke/webrtc_ice_smoke.py" "${ice_args[@]}"
  fi
