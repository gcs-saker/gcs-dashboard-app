...
usage() {
  cat <<'EOF'
Usage:
  scripts/m7_publish_play_smoke.sh [--check|--run]
...
run_webrtc_smoke() {
  docker run --rm \
    ...
    -w /workspace \
    "$PYTHON_IMAGE" \
    bash -lc "pip install aiortc >/tmp/aiortc-install.log && python scripts/webrtc_ice_smoke.py --run --require-video-frame --whep-url http://edge/webrtc/${STREAM_PATH}/whep --stun-url stun:turn-primary:3478"
}

run_check() {
  bash -n "$0"
  bash -n "${REPO_ROOT}/scripts/m7_single_node_runtime_smoke.sh"
