...
def verify_recovery(
    original: Path, restored: Path, anchor_path: Path, key_path: Path
) -> int:
    ...
    if (
        anchor.get("recordCount") != record_count
        or anchor.get("chainHead") != chain_head
    ):
...
def main() -> int:
    ...
    count = verify_recovery(
        args.original, args.restored, args.anchor, args.hmac_key_file
    )
